import { validateAndFormatColumnValues, validateCreateItemInput, } from "../types.js";
import { z } from "zod";
// Query Definitions
...
`;
export const createItemMutation = (hasColumnValues, hasGroupId) => `
...
export const CREATE_ITEM_TOOL = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            columnValues: {
                type: "object",
                description: "Optional column values for the new item (as key-value pairs)",
...
// Operation Functions
export async function getItems(monday, boardId) {
    try {
        ...
        if (response.data?.boards?.length > 0) {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify(response.data.boards[0].items_page.items, null, 2),
                    },
                ],
            };
        }
...
export async function getItem(monday, itemId) {
    try {
...
export async function createItem(monday, boardId, itemName, groupId, columnValues) {
    try {
        ...
        const mutation = createItemMutation(!!validatedInput.columnValues, !!validatedInput.groupId);
