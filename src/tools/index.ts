...
export function registerTools(server: McpServer) {
  server.tool(
    ...
    async ({ itemId }) => {
      ...
      const data = await mondayClient.makeRequest<ItemsResponse>(query);
      console.log(data);
      return {
        content: [
          {
  ...
  server.tool(
    "get-board",
    "Get a specific board by ID from Monday.com",
    {
  ...
  server.tool(
    "get-workspace",
    "Get a specific workspace by ID from Monday.com",
    {
    ...
    async ({ workspaceId }) => {
      ...
      const data = await mondayClient.makeRequest<WorkspacesResponse>(query);
      return {
        content: [
          {
            type: "text",
  ...
  server.tool(
    "get-group",
    "Get a specific group by ID and board ID from Monday.com",
    {
      boardId: z.string().describe("The ID of the board containing the group"),
  ...
  server.tool(
    "get-groups",
    "Get all groups in a board from Monday.com",
    {
      boardId: z.string().describe("The ID of the board to get groups from"),
    },
    async ({ boardId }) => {
      const query = `query { 
        boards (ids: [${boardId}]) { 
  ...
  server.tool(
    "search-items",
    "Search for items across all boards in Monday.com",
    {
      term: z.string().describe("The search term to look for"),
      limit: z
        .number()
        .optional()
    ...
    async ({ term, limit = 10 }) => {
      ...
      const data = await mondayClient.makeRequest<ItemsPageResponse>(query);
      return {
        content: [
          {
            type: "text",
  ...
  server.tool(
    ...
    async ({ boardId, groupId, itemName, columnValues }) => {
      ...
      const data = await mondayClient.makeRequest<{ create_item: Item }>(query);
      return {
        content: [
          {
            type: "text",
  ...
  server.tool(
    "preview-board-deletion",
    "Preview all items that will be deleted from a board",
    {
      boardId: z.string().describe("The ID of the board to clear"),
    },
    async ({ boardId }) => {
      const query = `query { 
        boards (ids: [${boardId}]) { 
...
To proceed with deletion, simply respond with "Yes"`,
  ...
  server.tool(
    "delete-board-items",
    "Delete all items from a board after confirmation",
    {
      boardId: z.string().describe("The ID of the board to clear"),
      confirmation: z.string().describe("Confirmation response (Yes/No)"),
    },
    async ({ boardId, confirmation }) => {
      ...
      const boardData =
        await mondayClient.makeRequest<BoardsResponse>(queryItems);
      ...
      const deleteData = await mondayClient.makeRequest<{
        delete_items: { count: number };
      }>(deleteMutation);
